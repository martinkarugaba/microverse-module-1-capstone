...
featuredSpeakersList.forEach((speaker) => {
  ...
  featuredSpeakersSection.innerHTML += `
    <article class='featuredSpeaker w-[100%] md:w-[45%] md:gap-x-[10px] flex justify-between items-center gap-x-4'>
      ...
      <div class='w-[60%]'>
        <h3 class='font-semibold text-lg text-primary-black md:text-lg'>${name}</h3>
        <p class='text-sm text-primary-orange font-semibold italic'>${jobTitle}</p>
        <div class="w-[30px] h-[2px] bg-primary-grey mt-2"></div>
        <p class='text-base text-gray-600 md:text-base mt-2'>${about}</p>
      </div>
    </article>
  `;
})
